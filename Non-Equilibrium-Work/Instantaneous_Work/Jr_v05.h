...
void calcInstWork(const dataFormat& df,
...
template <typename T>
void cumulateWork(const dataFormat& df, T** wIns, T** wAvg)
{
	for (int j = 0; j < df.numFolders * df.numSims; j++)
	{
		wAvg[0][j] = T(0);
		for (int i = 1; i < df.numDirs * df.numData; i++)
		{
			wAvg[i][j] = wIns[i][j] + wAvg[i - 1][j];
		}
	}
}
