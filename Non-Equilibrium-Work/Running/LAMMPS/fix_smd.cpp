...
enum { SMD_NONE=0,
       ...
       SMD_CVEL=1<<2, SMD_CFOR=1<<3, SMD_ROTATION=1<<8,
  ...
  Fix(lmp, narg, arg)
{
  ...
  if (strcmp(arg[argoffs],"tether") == 0) {
    ...
    if (r0 < 0) error->all(FLERR,"R0 < 0 for fix smd command");
    argoffs += 5;
  } else if (strcmp(arg[argoffs],"rotation") == 0) {
        if (narg < argoffs+4) error->all(FLERR,"Illegal fix smd command");
        styleflag |= SMD_ROTATION; 
        px    = utils::numeric(FLERR,arg[argoffs+3],false,lmp);
        py    = utils::numeric(FLERR,arg[argoffs+4],false,lmp);
        rz    = utils::numeric(FLERR,arg[argoffs+5],false,lmp);
        argoffs += 4;
  } else error->all(FLERR,"Illegal fix smd command");
...
void FixSMD::init()
{
  ...
  if (styleflag & SMD_DIRECTION) {  
	...
	r_old = r0;
	r_now = r0;
  } else if (styleflag & SMD_ROTATION) {
        xc = xcm[0];
        yc = xcm[1];
        zc = xcm[2];
	r_old = atan((yc-py)/(xc-px));
	r_now = atan((yc-py)/(xc-px));
        if (xc<px) {
             r_old += M_PI;
             r_now += M_PI;
        }
...
void FixSMD::post_force(int vflag)
{
  ...
  else if (styleflag & SMD_ROTATION)  smd_rotation();
  else smd_couple();
...
void FixSMD::smd_direction()
{
  ...
  double dx,dy,dz,fx,fy,fz,r,dr,drl;
...
void FixSMD::smd_rotation()
{
  ...
  double dx,dy,dz,fx,fy,fz,r,dr,drl,l;
