...
JarCalculator::JarCalculator(int argc, char **argv) {
  ...
  print_message("Usage: ");
  string message = std::string(argv[0]) +
                   " numFolders numSims numDataPerSim folder1 folder2 ....";
...
void JarCalculator::initialize() {
  ...
  z2.resize(numSims * numData * numFolders *
            numDirs); // forward-reverse + two folders
  z3.resize(numSims * numData * numFolders *
            numDirs); // forward-reverse + two folders
  f.resize(numSims * numData * numFolders *
           numDirs); // forward-reverse + two folders
  w.resize(numSims * numData * numFolders *
           numDirs);               // forward-reverse + two folders
...
void JarCalculator::readSims() {
  for (int z = 0; z < numFolders; z++)
    for (int i = 0; i < numSims; i++) {
      ...
      fileNames[0] = folders[z] + "/run-" + to_string((i + 1) * 1000000) +
                     "/forward/" + "smd.out";
      fileNames[1] = folders[z] + "/run-" + to_string((i + 1) * 1000000) +
                     "/reverse/" + "smd.out";
      ...
      for (int k = 0; k < 2; k++) {
        ...
        if (true) {
          ...
          while (getline(file, line)) {
            ...
            indx = i + z * numSims +
                   numFolders * numSims *
                       (j + k * numData); // (i+ z * numSims, j + k * numData )

            if (i == 0 && z == 0 && j < numData) {
              int stp = (k == 0) ? outputFreq * j
                                 : outputFreq * (j + k * numData - 1);
        ...
        if (j < numData) {
          string message = "Warning: Only " + std::to_string(j) +
                           " data lines instead of were read from " +
                           fileNames[k];
          print_message(message);
        ...
        if (j > numData) {
          string message =
              "Warning: Discarding " + std::to_string(j - numData) +
              " from the end of forward and begining of reserve for " +
              fileNames[k];
          print_message(message);
        }
...
void JarCalculator::writeData(const vector<double> &data,
  ...
  for (int z = 0; z < 1; z++)
    for (int j = 0; j < 2 * numData; j++) {
      ...
      int totExcept = 0;
      for (int k = 0; k < numFolders * numSims; k++)
        totExcept += exceptions[k];
...
void JarCalculator::calcWork() {
  ...
  for (int i = 0; i < numFolders * numSims; i++) {
    ...
    for (int j = 1; j < 2 * numData; j++) {
      int indx1 = i + numFolders * numSims *
                          (j - 1); // row based [i,j-1] index with 2*numData
                                   // rows and numFolders*numSims columns
      int indx2 =
          i + numFolders * numSims * j; // row based [i,j] index with // // //
...
void JarCalculator::calcBar() {
  ...
  string line;
  stringstream iss;
  std::string command = "./BAR-v02.py " + std::to_string(numFolders) + " " +
                        std::to_string(numSims) + " " + std::to_string(numData);
...
#pragma omp critical
    ...
    double gBar_meanI =
        std::accumulate(gBarShuffleI, gBarShuffleI + numShuffles, 0.0,
                        [&count, etol](double acc, double val) {
                          if (std::abs(val) >= etol) {
                            count++;
                            return acc + val;
                          } else
...
void JarCalculator::writeBarShuffled() {
  ...
  if (output.is_open())
    cerr << "Cannot open file " << gBarShuffle_filename << " for writing"
         << endl;
...
void JarCalculator::writeOutput() {
  ...
  for (int i = 0; i < 2 * numData; i++) {
    file << step[i] << "," << z1avg[i] << "," << gExp[i] << "," << gSec[i]
         << "," << gBar[i] << "," << wAvg[i] << endl;
  }
  file.close();
}
  ...
  for (int j = 0; j < numFolders; j++)
    for (int i = 0; i < numSims; i++) {
      ...
      if (exceptions[indx] == 1)
        file << (j + 1) << "-Series-" << (j + 1) << "/run-" << (i + 1) * 1000000
             << endl;
    }
  file.close();
}
...
void JarCalculator::shuffleIndexes() {
  shuffles.resize(numSims * numFolders *
                  numShuffles); // row-based indexing with shuffles in rows and
                                // numFolders*numSims in columns so each shuffle
                                // in the continguous memory locations.
