...
backward_works = df.iloc[num_data:,1:last_col] - df.iloc[-1,1:last_col]
...
for index, row in forward_works.iterrows():
    fw1 = forward_works.iloc[index,:].values
    bw1 = backward_works.iloc[index,:].values
    
    fw = [x/0.592 for x in fw1]
    bw = [-x/0.592 for x in bw1]
    ...
    results = pymbar.other_estimators.bar(fw,bw,maximum_iterations=2000)
...
with open("BAR-v02.dat","w") as csvfile:
