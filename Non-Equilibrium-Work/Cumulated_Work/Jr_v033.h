...
enum class modeVal { NORMAL = 0, NUMBER_ANALYSIS = 1 << 1 };
...
public:
  ...
  JarCalculator(const string &logName_, const int &numFolders_,
                const int &numSims_, const int &numData_, Args &&...args);
  JarCalculator(const JarCalculator &input) = delete;
  JarCalculator(const JarCalculator &&input) = delete;
  JarCalculator &operator=(const JarCalculator &rhs) = delete;
  JarCalculator &operator=(const JarCalculator &&rhs) = delete;
  ~JarCalculator() { print_message("All done!"); }
...
private:
  ...
  void readSims();
  void writeData(const vector<double> &data, const string &fileName,
